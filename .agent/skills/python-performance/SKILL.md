...
## 5. Long-Term Performance Tracking (Advanced Pattern)

For production systems like Andrew Martin, it is essential to monitor performance over time.

### Persistent Logger Pattern
Instead of just printing to the console, use a persistent logger that saves metrics to a JSON file.
...
class PerformanceLogger:
    ...
    def log_metric(self, name, duration, metadata=None):
        entry = {
...
### Integrated Benchmark Decorator
Connect your `@benchmark` decorator to the `PerformanceLogger` to capture every execution.
...
def benchmark(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        result = func(*args, **kwargs)
        elapsed = time.perf_counter() - start
        performance_logger.log_metric(func.__name__, elapsed)
        return result
    return wrapper
```
