...
try:
    ...
    safe_print("✅ Todos los módulos importados correctamente.")
except ImportError as e:
    safe_print(f"❌ Error de importación: {e}")
    sys.exit(1)
...
if len(tool_list) == 0:
    safe_print("❌ No se registraron herramientas.")
    sys.exit(1)
...
if missing:
    safe_print(f"❌ Faltan herramientas: {missing}")
    sys.exit(1)
else:
    safe_print("✅ Todas las herramientas esenciales están registradas.")
...
try:
    ...
    if "usuarios" in result:
        safe_print("✅ list_users devolvió estructura esperada.")
    else:
        safe_print("⚠️  list_users devolvió un formato inesperado.")
except Exception as e:
    safe_print(f"❌ Error al ejecutar list_users: {e}")
    sys.exit(1)
...
try:
    ...
    if result.get("success") == True:
        safe_print(f"✅ datetime funciona: {result.get('human_readable', '')}")
    else:
        safe_print(f"⚠️  datetime no tuvo éxito: {result.get('error', '')}")
except Exception as e:
    safe_print(f"❌ Error al ejecutar datetime: {e}")
...
try:
    ...
    safe_print("✅ agents.py importado correctamente.")
except Exception as e:
    safe_print(f"❌ Error importando agents.py: {e}")
    sys.exit(1)
